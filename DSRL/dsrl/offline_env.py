...
from safety_gymnasium.wrappers import gymnasium as gymn
import h5py
import numpy as np
...
def wrap_env(
    ...
    env = gymn.wrappers.TransformObservation(env, normalize_state)
    if reward_scale != 1.0:
        env = gymn.wrappers.TransformReward(env, scale_reward)
    return env
...
class NormalizationEnvWrapper(gymn.Wrapper, OfflineEnv):
    ...
    def __init__(self, env,
        ...
        self.is_cost_separate = is_cost_separate
