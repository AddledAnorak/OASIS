...
def shoan_augmentation(trajs: list,
                 deg: int = 3,
                 max_rew_decrease: float = 1,
                 beta: float = 1,
                 augment_percent: float = 0.3,
    ...
    ''' Alternative Implementation without using OApackage '''
    objectives = np.stack((-cost_ret, rew_ret), axis=1)
    def is_pareto_efficient(points):
        """
        Find the Pareto-efficient points.
        Returns a boolean array where True indicates Pareto-efficient points.
        """
        is_efficient = np.ones(points.shape[0], dtype=bool)
        for i, point in enumerate(points):
            if is_efficient[i]:
                # Compare against all other points
                is_efficient[is_efficient] = ~np.all(points[is_efficient] <= point, axis=1) | (points[is_efficient] == point).all(axis=1)
                is_efficient[i] = True  # Keep the current point as Pareto-efficient
        return is_efficient

    pareto_mask = is_pareto_efficient(objectives)
    pareto_idx = list(np.where(pareto_mask)[0])
...
class SequenceDataset(IterableDataset):
    ...
    def __init__(
        ...
        if pf_only:
            ...
            self.idx, self.aug_data, self.pareto_frontier, self.indices = shoan_augmentation(
